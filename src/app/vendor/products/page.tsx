...
export default function VendorProductsPage() {
  ...
  const toggleStock = (id: string) => {
    setProducts((prev) =>
      prev.map((p) => (p.id === id ? { ...p, stock: p.stock > 0 ? 0 : 100 } : p))
    );
  };
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total', value: products.length, color: 'bg-gray-100 text-gray-800' },
          { label: 'In Stock', value: products.filter((p) => p.stock > 50).length, color: 'bg-emerald-100 text-emerald-800' },
          { label: 'Low Stock', value: products.filter((p) => p.stock > 0 && p.stock <= 50).length, color: 'bg-amber-100 text-amber-800' },
        ...
        <Table headers={['Product', 'Category', 'Price', 'Stock', 'Status', 'Actions']}>
          {filtered.map((product) => (
            <tr key={product.id} className="hover:bg-gray-50 transition-colors">
              ...
              <Td>
                <span className={`font-semibold ${product.stock > 50 ? 'text-emerald-600' : product.stock > 0 ? 'text-amber-600' : 'text-red-600'}`}>
      ...
      >
        ...
        <div className="flex gap-3 justify-end">
          ...
          <Button variant="danger" onClick={() => deleteTarget && handleDelete(deleteTarget.id)}>Delete</Button>
        </div>
      </Modal>
    </div>
  );
